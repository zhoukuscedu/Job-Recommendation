...
public class MonkeyLearnClient {
    ...
    private static final String AUTH_TOKEN = "db7f7e5314e16e617bda43b479db961dda6ecbb4";

    public List<Set<String>> extract(List<String> articles){
        //json mapping and closeable client
        ObjectMapper mapper = new ObjectMapper();
        CloseableHttpClient httpClient = HttpClients.createDefault();

        //new request with auth token
        HttpPost request = new HttpPost(EXTRACT_URL);
        request.setHeader("Content-type", "application/json"); //content header
        request.setHeader("Authorization", "Token " + AUTH_TOKEN); //auth header

        //extract request body from list of strings
        ExtractRequestBody body = new ExtractRequestBody(articles, 3);

        String jsonBody;

        //try to use json mapping on body
        try{
...
//Function to test the MonkeyLearn Client extractor function
